...
- [ ] 0.1 Clean tree on main; re-verify the file:line anchors in design.md
      Context before editing (they were verified 2026-07-23 and may drift).

## 1. Shared schema + seed manifest

- [ ] 1.1 Create `src/shared/model-catalog.ts`: zod schema (strict) + TS types
...
- [ ] 1.2 Create `docs/model-catalog.json` seeding the current built-in
      ...
      change.
- [ ] 1.3 Unit tests: schema rejects unknown keys, oversize strings, bad id
      charset; merge adds/updates/deprecates and never drops a built-in.

## 2. Main-process fetcher + router

- [ ] 2.1 Create `src/main/lib/model-catalog/fetcher.ts` modeled on
...
- [ ] 2.2 Local-only mode: gate on the raw boolean `isLocalOnlyMode()`
      ...
      guard-check error.
- [ ] 2.3 New `src/main/lib/trpc/routers/model-catalog.ts` with a `get` query
...
- [ ] 2.4 Tests: TTL respected, invalid remote falls back to cache then
      built-ins, local-only skips network (assert fetchImpl never called),
      cache file permissions.
- [ ] 2.5 Add `src/main/lib/model-catalog` to `RUNTIME_CORE_DIRECTORIES` in
...
- [ ] 3.1 New renderer store (Jotai atom + tRPC query, pattern:
...
- [ ] 3.2 Deprecated models: hidden from picker lists; a persisted selection
...
- [ ] 3.3 Custom model entry: "Custom model…" row in Claude and Codex pickers
...
- [ ] 3.4 Remove the clamps: `ipc-chat-transport.ts` sends non-alias ids
...
- [ ] 3.5 Tests: transport passes custom ids through unchanged (both
      runtimes); alias mapping unchanged; deprecated hidden but persisted
      selection survives.

## 4. OpenAI /v1/models parse (Codex API-key source)

- [ ] 4.1 `codex/api-key-validation.ts`: today the success branch returns on
...
- [ ] 4.2 Codex picker: when active source is openai-api-key, show cached live
      ids absent from the merged catalog under an "available via your API key"
      group (custom-id styling, no metadata).
- [ ] 4.3 Tests: parse tolerates malformed body; group only renders for the
      api-key source; catalog entries never duplicated in the extra group.

## 5. Verification

- [ ] 5.1 `bun run check` green (includes the import-boundary guard);
      record counts in `verification.md`.
- [ ] 5.2 Manual smoke, recorded in `verification.md`: (a) fresh launch
      ...
      requests.
- [ ] 5.3 `openspec validate add-remote-model-catalog --strict
