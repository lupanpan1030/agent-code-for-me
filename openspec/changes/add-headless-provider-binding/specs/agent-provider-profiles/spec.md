...
The runtime-purpose provider defaults (`claude-main`, `codex-main`) SHALL be consumed by headless and scheduled runs as the default provider source when a run specifies no explicit provider, including the default's model override. Desktop interactive runs keep their existing per-chat selection semantics; setting a headless default does not override first-party auth for runs that never resolve to it.
...
- **AND** a headless run starts without an explicit provider selection
