...
- Resolution per run: an explicit profile uses that profile; a model-only `provider` block stays native; only omission of the entire `provider` object consults `agent_provider_defaults` purposes `claude-main`/`codex-main` (decision D3: headless is the consumer that gives those rows meaning) before native runtime credentials.
...
- Affected specs: `local-job-api` (provider selection by reference), `headless-agent-jobs` (CLI/schedule selection + resolution semantics), `provider-runtime-bindings` (headless binding, token lifecycle, and exact output redaction), `provider-credential-storage` (credential input and authoritative stored-value validation), `agent-provider-profiles` (headless default consumption only)
