...
- [ ] 1.1 Verify the keep-list in `proposal.md` → "Explicitly NOT changed" by reading, not
...
- [ ] 1.2 C7 consumer sweep for the stdio surface: re-confirm
...
- [ ] 1.3 Capture a baseline `bun run check:full` receipt on the pre-change commit so later
      failures are attributable.

## 2. Specs first (repo convention — no code until validated)

- [ ] 2.1 Land the three delta files in this change directory: `agent-protocol-interfaces`
      (RENAMED + MODIFIED), `canonical-entity-vocabulary` (ADDED), `fork-residue-hygiene` (ADDED).
- [ ] 2.2 `openspec validate refactor-engine-vocabulary-residue --strict --no-interactive` → valid;
      repo-wide `--changes` and `--specs` also pass.

## 3. Single engine-id source (`src/shared/`)

- [ ] 3.1 `agent-chat-provider.ts`: replace the independent literal at `:1` with a derivation from
...
- [ ] 3.2 Extend `scripts/check-architecture-guards.mjs` per design.md Decision 3: the module must
...
- [ ] 3.3 Optional gated rename (design.md Decision 2): `AgentChatProvider` → `ChatEngineId`
...
- [ ] 3.4 `tsc --noEmit` green.

## 4. Transport rename (renderer, zero persistence)

- [ ] 4.1 Rename `src/renderer/features/agents/lib/acp-chat-transport.ts` →
...
- [ ] 4.2 Update the consumers: `active-chat.tsx` (~8 refs: import at `:184`, `instanceof` at
...
- [ ] 4.3 Update `docs/OWNERSHIP_MAP.md` "Runtime Chat UI Event State" consumers row (`:59`) to the
      new filename.
...
- [ ] 4.4 Re-point the six source-text assertion suites that read the old file path via
...
- [ ] 4.5 Update the 1b-installed guard references in `scripts/check-architecture-guards.mjs`
...
- [ ] 5.1 Rename `NewChatAgent` → `NewChatEngine` (`:195`), `agents` → `engines` (`:202`),
...
- [ ] 5.2 Delete the retired disabled `"cursor"` entry (`:204`) and the `| "cursor"` union member
...
- [ ] 6.1 Rename `src/main/lib/headless/acp-stdio.ts` → `jobs-stdio.ts`;
...
- [ ] 6.2 `cli-args.ts`: `kind: "acp"` → `"jobs-stdio"` (`:164`), command parse `"acp"` →
      ...
      case (`:1144-1145`).
- [ ] 6.3 Confirm no migration is needed: nothing reads `agent_jobs.input.protocol` for routing;
      historical `locus-acp-stdio.v1` rows stay untouched. Record the confirming grep in
      `verification.md`.
- [ ] 6.4 Docs: update ~20 `locus acp` / `locus-acp-stdio` references across ~10 files —
...
- [ ] 7.1 Characterize existing behavior first: unit-test matrix against the current regex
...
- [ ] 7.2 Create `src/shared/worktree-path.ts` (pure string logic, no node imports; design.md
      Decision 5): marker constant, `isManagedWorktreePath()`,
      `parseManagedWorktreeRelativePath()`.
- [ ] 7.3 Replace and DELETE the five copies in the same change:
      ...
      marker constant.
- [ ] 7.4 `agents-project-worktree-tab.tsx`: remove `"1code"` from the `saveTarget` union (`:278`,
...
- [ ] 8.1 `bun run check:full` green; compare test counts against the 1.3 baseline and account for
      every added test.
- [ ] 8.2 Negative residue greps (excluding `openspec/changes/archive/**`): zero hits for
...
- [ ] 8.3 Untouched-surface proof: `acp-tool-normalizer.ts`, `chat-message-normalizer.ts`,
...
- [ ] 8.4 Desktop smoke (`bun run dev`), recorded in `verification.md`: engine picker offers
      ...
      worktree.
- [ ] 8.5 `openspec validate --strict --no-interactive` across all changes and specs.
