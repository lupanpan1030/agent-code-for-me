...
describe("quick chat new chat form", () => {
  test("gates folderless provider choices through runtime capability truth", () => {
    ...
    expect(form).toContain("isEngineOptionDisabled(engine)")
    ...
    expect(form).toContain("selectedEngineIsRuntimeAllowed")
    expect(form).toContain('{ id: "claude-code", name: "Claude Code"')
    expect(form).toContain('{ id: "codex", name: "OpenAI Codex" }')
    expect(form).not.toContain('id: "cursor"')
    expect(form).not.toContain("Cursor CLI")
