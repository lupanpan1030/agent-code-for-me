...
import {
  ...
  type WorktreeSetupTrustRequest,
} from "./worktree-setup-trust"

const execFileAsync = promisify(execFile)
export const WORKTREE_CREATE_TIMEOUT_MS = 300_000
export const WORKTREE_CREATE_TIMEOUT_SECONDS = WORKTREE_CREATE_TIMEOUT_MS / 1000
...
export type WorktreeSetupApprovalRequired = WorktreeSetupTrustRequest & {
  chatId: string
  worktreePath: string
}
...
export async function createWorktreeForChat(
  ...
  try {
    ...
    if (options?.projectId) {
      ...
      if (setupTrust.status === "approved") {
        executeWorktreeSetupCommands(
          worktreePath,
          projectPath,
          setupTrust.request.commands,
        )
          .then((setupResult) => {
            ...
            if (!setupResult.success) {
              ...
              console.log(
                `[worktree] Setup completed successfully for ${chatId}`,
              )
            }
          })
          .catch((setupError) => {
      ...
      } else if (setupTrust.status === "untrusted") {
        options.onSetupApprovalRequired?.({
          ...setupTrust.request,
          chatId,
          worktreePath,
        })
