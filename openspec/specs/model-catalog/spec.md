...
## Purpose
Define the bounded model-discovery layer that combines an offline built-in floor, a strictly
validated last-good remote metadata overlay outside local-only mode, custom-model pass-through,
and provider-discovered Codex API-key models without treating the catalog as a runtime allowlist.
