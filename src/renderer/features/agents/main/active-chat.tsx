...
export function ChatView({
  ...
  // Subscribe to activeSubChatId for plan details (needs to update when switching sub-chats)
  ...
  }, [openDetailsWidget, setIsTerminalSidebarOpen, terminalDisplayMode])
  ...
  const handleToggleTerminalProductEntry = useCallback(() => {
    ...
    openDetailsWidget("terminal", { toggle: true })
  ...
  const canShowDiffButton = !!worktreePath
...
Make sure to preserve all functionality from both branches when resolving conflicts.`
  ...
  const codexMcpSessionData = useMemo(() => {
    ...
    for (const server of effectiveServers.values()) {
      ...
      mcpServers.push({
        name: server.name,
        status,
        ...("serverInfo" in server && server.serverInfo
  ...
  }, [activeSubChatId, agentChat, getOrCreateChat, chatId, worktreePath])
  ...
  return (
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-hidden flex">
          ...
          {!shouldHideChatHeader && (
            ...
            >
              ...
              <div className="pointer-events-auto flex items-center justify-between relative">
                ...
                {/* Details Button - shows when sidebar is closed and worktree/sandbox exists (desktop only) */}
                {!isMobileFullscreen &&
                  worktreePath &&
        ...
        {/* Details Sidebar - canonical right inspector for current chat details */}
        {!isMobileFullscreen &&
          worktreePath && (
          <DetailsSidebar
            chatId={chatId}
